...
class DashboardController < ApplicationController
  ...
  def persona_post
    ...
    Rails.logger.debug("--------------BOTON> " + @registrar)
    ...
    if @registrar=="Guardar"
      ...
      pers.save
    elsif @registrar=="Actualizar"
      
      Rails.logger.debug("--------------INI> " + @nro_documento)
      #pers = Persona.find_by(nro_documento: @nro_documento)
      Rails.logger.debug("--------------FIN> " + @nro_documento)
      Persona.where('nro_documento = ?', @nro_documento).update_all(nombre: @nombre, ape_paterno: @ape_paterno, ape_materno: @ape_materno, email: @email, password: @password, celular: @celular, direccion: @direccion)
    end
    
    #render action: :persona
    #redirect_to :back
    
    redirect_to :action => "persona"
